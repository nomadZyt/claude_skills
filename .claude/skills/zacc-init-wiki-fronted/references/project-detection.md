...
命中 `package.json` 后，额外检查是否为 Taro 小程序项目：

| 检测信号 | 检查方式 | 优先级 |
|---------|---------|--------|
| `package.json` dependencies 含 `@tarojs/taro` | 读取 `dependencies` + `devDependencies` | 1（最强） |
| `package.json` dependencies 含 `@tarojs/cli` | 读取 `devDependencies` | 2 |
| 存在 `config/` 目录且含 `index.ts`/`index.js` | Glob `config/index.*` | 3 |
| 存在 `project.config.json`（微信小程序配置） | 检查根目录文件 | 4 |
| `app.config.ts` / `app.config.js`（Taro 入口配置） | 检查 `src/` 下文件 | 5 |

**判定规则**：
- 信号 1 或 2 命中 → 确认为 Taro 小程序，`project_type = taro-miniprogram`
- 仅信号 3/4/5 命中 → 标记为 `project_type = taro-miniprogram`，但 `confidence: medium`，需进一步确认
- 如同时存在 `@tarojs/taro` 和 Web 框架（如 `react-dom`），检查 `package.json` 的 `scripts` 中是否有明确的编译目标：
  - 含 `build:weapp` / `build:swan` / `build:alipay` 等 → 多端 Taro 项目，`project_type = taro-multi-platform`
  - 含 `build:h5` 但无小程序目标 → Taro H5 项目，按普通前端处理

**Taro 项目特有属性**：

| 属性 | 值 | 说明 |
|------|---|------|
| `language` | TypeScript / JavaScript | 同普通前端检测 |
| `entry_file` | `src/app.config.ts` > `src/app.ts` > `src/app.tsx` | Taro 入口为 app.config + app |
| `source_root` | `src/` | Taro 标准源码目录 |
| `framework` | `react` / `vue` / `vue3` | 从 `package.json` 的 `@tarojs/plugin-framework-*` 推断 |

**框架推断**：
- `@tarojs/plugin-framework-react` 或 `dependencies` 含 `react` → `framework: react`
- `@tarojs/plugin-framework-vue3` 或 `dependencies` 含 `vue`（版本 3.x）→ `framework: vue3`
- `@tarojs/plugin-framework-vue2` → `framework: vue2`
