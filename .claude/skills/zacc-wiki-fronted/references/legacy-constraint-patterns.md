...
使用 Grep 检测以下信号。当 AI 在编码过程中遇到这些信号时，应建议用户通过 **zacc-wiki-fronted** 选「增量更新 → **legacy**」记录。
...
2. 用 AskUserQuestion 提示：
   ...
   4. 跳过，不记录
   ```
3. 根据用户选择，引导其通过 **zacc-wiki-fronted** 选「增量更新 → **legacy**」
4. 继续原来的编码任务

## 批量扫描模式

执行 **zacc-wiki-fronted** 选「增量更新 → **scan**」时，可选启用批量历史包袱扫描：
